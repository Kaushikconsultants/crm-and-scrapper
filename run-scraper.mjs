...
async function run() {
  const categories = ["Restaurants", "Hospitals", "Schools", "Gyms", "Hotels", "Salons", "Clinics", "Real Estate"];
  const locations = ["Delhi", "Haryana"];
  const maxLeadsTotal = 500;

  console.log(`Starting standalone scraper for multiple categories in ${locations.join(' and ')}`);
  ...
  try {
    ...
    for (const location of locations) {
      ...
      while (totalExtracted < maxLeadsTotal && unchangedCount < 4 && !isEndOfList) {
        console.log(`Scrolling feed for ${category}... (Found ${totalExtracted}/${maxLeadsTotal} total leads so far)`);
          ...
          if (finalBatch.length > 0) {
              ...
              for (let i = 0; i < finalBatch.length; i++) {
                  ...
                  try {
                      ...
                      await new Promise(r => setTimeout(r, 3500)); // give React time to render
    ...
    console.log(`\nScraping complete! Found ${totalExtracted} new unique leads.`);
  } catch (error) {
